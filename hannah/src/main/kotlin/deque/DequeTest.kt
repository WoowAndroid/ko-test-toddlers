...
fun testForDequeByJava() {
    val deque = java.util.ArrayDeque<Int>()

    deque.addFirst(1) // deque [1]
    deque.add(2) // deque [1, 2]
    deque.addLast(3) // deque [1, 2, 3]
    deque.offerFirst(4) // deque [4, 1, 2, 3]
    ...
    deque.pollFirst() // pollFirst(...) must not be null
    deque.pollLast() // pollLast() must not be null
    deque.peekFirst() // peekFirst(...) must not be null
    deque.peekLast() // peekLast(...) must not be null
